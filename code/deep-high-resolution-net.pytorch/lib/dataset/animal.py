...
class AWADataset(JointsDataset):
    """
    "keypoints": {
        0: "left_ear",
        1: "right_ear",
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        with open("data/quadruped_keypoints/coco_format/flip_pairs.pickle", "rb") as handle:
            self.flip_pairs = pickle.load(handle)

        self.parent_ids = None
        # self.upper_body_ids = (0, 1, 2, 3, 4, 5, 6)
        with open("data/quadruped_keypoints/coco_format/upper_body_ids.pickle", "rb") as handle:
            self.upper_body_ids = pickle.load(handle)

        # self.lower_body_ids = (7, 8, 9, 10, 11, 12, 13, 14)
        with open("data/quadruped_keypoints/coco_format/lower_body_ids.pickle", "rb") as handle:
            self.lower_body_ids = pickle.load(handle)
        ...
        with open("data/quadruped_keypoints/coco_format/joints_weight.pickle", "rb") as handle:
            self.joints_weight = pickle.load(handle)

        self.joints_weight = np.array(self.joints_weight, dtype=np.float32).reshape(
            (self.num_joints, 1)
        )
    ...
    def _get_ann_file_keypoint(self):
        ...
        prefix = "keypoints" if "test" not in self.image_set else "image_info"
        return os.path.join(
            self.root,
            "quadruped_keypoints",
            "coco_format",
            "supervised_dataset_" + self.image_set + "_5.pickle",
        )
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        for obj in objs:
            # x, y, w, h = obj['bbox']
            x, y, w, h = obj["clean_bbox"]

            # if index == 'pig_10012':
            #    print('animal.py line 198: bbox = ', obj['clean_bbox'])
            ...
            if obj["area"] > 0 and x2 >= x1 and y2 >= y1:
                ...
                obj["bbox_x1y1x2y2"] = np.array([[x1, y1], [x2, y2]])
                valid_objs.append(obj)

            # if index == 'pig_10012':
            #   print('animal.py line 210: bbox = ', obj['bbox_x1y1x2y2'])
    ...
    def image_path_from_index(self, index):
        ...
        folder_name = index.split("_")[0]

        image_path = os.path.join(self.root, "JPEGImages", folder_name, file_name)
    ...
    def _load_coco_person_detection_results(self):
        ...
        logger.info(
            "=> Total boxes after fliter low score@{}: {}".format(self.image_thre, num_boxes)
        )
        return kpt_db
