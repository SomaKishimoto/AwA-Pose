...
def save_demo_images(batch_image, batch_joints, batch_joints_vis, nrow=1, padding=0):
    """
...
def save_batch_image_with_joints_n_bbox(
    batch_image, bboxs, batch_joints, batch_joints_vis, file_name, nrow=8, padding=2
):
    """
    batch_image: [batch_size, channel, height, width]
    batch_joints: [batch_size, num_joints, 3],
...
def save_batch_heatmaps(batch_image, batch_heatmaps, file_name, normalize=True):
    ...
    grid_image = np.zeros(
        (batch_size * heatmap_height, (num_joints + 1) * heatmap_width, 3), dtype=np.uint8
    )
...
def save_debug_images_w_bbox(config, input, meta, target, joints_pred, output, prefix):
    if not config.DEBUG.DEBUG:
        return

    if config.DEBUG.SAVE_BATCH_IMAGES_GT:
        save_batch_image_with_joints_n_bbox(
            input, meta["bbox"], meta["joints"], meta["joints_vis"], "{}_gt.jpg".format(prefix)
        )
    if config.DEBUG.SAVE_BATCH_IMAGES_PRED:
        save_batch_image_with_joints(
            input, joints_pred, meta["joints_vis"], "{}_pred.jpg".format(prefix)
